Create Database Ayodhya_Ram_Temple_SQLanalysis;

Use Ayodhya_Ram_Temple_SQLanalysis;
...
Select Name,PhaseName,StartDate,CompletionDate from Temple t join ConstructionPhase cp on t.TempleID = cp.TempleID;

/*3)Find the total amount of donations received for the Ram Mandir?*/
Select sum(amount) as Total_amount_of_donations, concat(round(sum(amount)/100000)," Lakhs") as in_readable_terms from Donations;

/*4)Get details about the architecture of the Ram Mandir?*/
Select Name,ArchitectName,Description from Temple t join Architecture a on t.TempleID = a.TempleID;

/*5)Retrieve events associated with the Ram Mandir?*/
Select Name,EventName,EventDate, Description from Temple t join TempleEvents te on t.TempleID = te.TempleID
														   join Events e on te.EventID = e.EventID;
...
Select Name, count(EventName) as Count_of_events from Temple t join TempleEvents te on t.TempleID = te.TempleID
														   join Events e on te.EventID = e.EventID group by name;
                                                           
/*10)Find the donors who made contributions on or after 2021-06-01'?*/
Select DonorName, DonationDate from Donations where DonationDate > "2021-06-01";
