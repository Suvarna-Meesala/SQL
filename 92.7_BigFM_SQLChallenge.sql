Create Database Radio_92_7_BigFM__SQLChallenge;

Use Radio_92_7_BigFM__SQLChallenge;
...
/*Question 1: Retrieve all stations in the "East" region?*/
select * from Stations where Location = "East" ;

/*Question 2: List all shows hosted by "Vrajesh Hirjee"?*/
Select * from shows s join hosts h on s.hostid = h.hostid where hostname = "Vrajesh Hirjee";

/*Question 3: Count the number of awards each show has won?*/
Select Showname, count(*) as No_of_awards from awards a join shows s on s.showid = a.showid group by awardid;

/*Question 4: Find shows that have partnerships with "Spotify"?*/
Select showname,Partnername from Shows s join Showpartnerships sp on s.showid=sp.showid join Partnerships p on p.partnershipid = sp.partnershipid where Partnername = "Spotify" ;

/*Question 5: Retrieve hosts who joined before 2010?*/
Select Hostname, Joindate from hosts where joindate < '2010-01-01';

/*Question 6: List the shows and their launch dates in descending order of launch date?*/
Select Showname, Launchdate from shows order by Launchdate desc;

/*Question 7: Find the total count of shows for each host?*/
Select Hostname, showcount as Total_count_of_shows from hosts group by Hostname,showcount order by showcount desc;

/*Question 8: Show the online presence platforms with their links?*/
Select PlatformName, Link from OnlinePresence;

/*Question 9: Retrieve the stations in the "South" region launched after 2010?*/
Select StationName,LaunchDate from Stations where Location="South" and LaunchDate >'2010-12-31';

/*Question 10: Rank hosts based on the number of shows they have hosted?*/
Select Hostname, showcount as Total_count_of_shows,Rank() over (order by max(showcount) desc) as Rank_of_Host from hosts group by Hostname,showcount order by showcount desc;
