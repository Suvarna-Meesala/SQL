Create Database Stranger_Things_SQLchallenge;

Use Stranger_Things_SQLchallenge;
...
Select Name, Event_name as Events from Relationships r join characters c on c.character_id = r.character1_id or c.character_id = r.character2_id 
											 join events e on e.event_id = character1_id or e.event_id = character2_id order by Name;

/*6)Calculate the total number of characters from each hometown?*/
Select Hometown, count(name) as No_of_characters from characters group by hometown;

/*7)Retrieve characters who were involved in events in Season 1 or Season 2?*/
Select Distinct(Name) as Name from Relationships r join Characters c on c.Character_id = r.Character1_id or c.Character_id = r.Character2_id 
										 join Events e on e.event_id = character1_id or e.event_id = character2_id where season <=2 ;

/*8)Find the top 3 oldest characters?*/
Select Name as Top_3_oldest_characters from characters order by age desc limit 3;

/*9)Find the average age of characters in Hawkins?*/
Select round(avg(age)) as Average_age from characters where hometown="hawkins" group by hometown;

/*10)Rank of characters by age in descending order?*/
Select name, rank()over (order by age desc) as Rank_of_character from characters group by name,age;