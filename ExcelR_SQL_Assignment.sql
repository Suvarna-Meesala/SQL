Create Database ExcelR_SQL_Assignment;

Use ExcelR_SQL_Assignment;
...
('S72_3212','Pont Yacht','Ships','1:72','Unimax Art Galleries','Measures 38 inches Long x 33 3/4 inches High. Includes a stand.\r\nMany extras including rigging, long boats, pilot house, anchors, etc. Comes with 2 masts, all square-rigged',414,'33.30','54.60');
														...
														//////////* DAYWISE ASSIGNMENT ANSWERS *//////////   


/*Day3* "Question1"*/

Select CustomerNumber,customername,state,creditlimit from customers where (creditlimit between 50000 and 100000 and State is not null) order by creditlimit desc;
...
Select  Distinct year(PaymentDate) as year, Min(Amount) as Min_Amount from Payments group by Year having Min_Amount > 0 order by Year(PaymentDate) asc;
...
from Orders group by Year,Quarter having Count(Distinct(CustomerNumber))>0 and Count(OrderNumber)>0 order by Year(OrderDate) asc ;


/*Day5* "Question3"*/

Select Distinct(Left(MonthName(PaymentDate),3)) as Month, CONCAT(Round(sum(amount) / 1000), 'K') as Formatted_Amount from Payments 
group by Month having Sum(Amount)<1000000 and Sum(Amount)>500000 order by sum(Amount) desc;
...
Bus_ID INT Not Null,
Bus_Name CHAR(20) NOT NULL,
Source_Station VARCHAR(20) NOT NULL, 
Destination VARCHAR(20) NOT NULL,
Email varchar(30) Unique NOT NULL
);

Describe Journey_Table ;
...
Select EmployeeNumber, Concat(FirstName,' ',LastName) as SalesPerson, Count(CustomerNumber) as UniqueCustomers from Employees join Customers on EmployeeNumber = salesRepEmployeeNumber
group by EmployeeNumber,SalesPerson order by Count(CustomerNumber) desc,EmployeeNumber;


/*Day7* "Question2"*/

Select 
C.CustomerNumber,C.CustomerName,P.ProductCode,P.ProductName,OD.QuantityOrdered as OrderedQuantity,sum(P.Quantityinstock) as TotalInventory,(P.QuantityinStock-OD.QuantityOrdered) as LeftQty 
...
Group by C.CustomerNumber,P.ProductCode,OrderedQuantity
...
Insert into Laptops values('Dell'),('HP');
...
Select * from Laptops, Colours order by Laptop_Name Asc;
...
SELECT P2.FullName AS ManagerName,P1.FullName as EmployeeName 
FROM Project AS P1 LEFT JOIN Project AS P2
ON P1.ManagerID=P2.EmployeeID where P2.FullName is not null order by P1.ManagerID,P2.EmployeeID asc;
...
Alter Table Facility Modify Facility_ID INT Primary key Auto_Increment NOT NULL;
...
UPDATE University
SET Name = Concat(REPLACE(Name, 'University', ' '),' ','University') ;
...
Select Distinct(year(O.OrderDate)) as Year, CONCAT(Count(od.QuantityOrdered),' (',ROUND(SUM(od.QuantityOrdered) / (SELECT SUM(QuantityOrdered) FROM OrderDetails) * 100, 0),'%)') as Value 
from OrderDetails as OD join Orders AS o on OD.OrderNumber = O.OrderNumber group by Year(O.OrderDate) Order by Count(od.QuantityOrdered) desc;
...
Delimiter $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetCustomerLevel`(Par_CustomerNumber INT)
BEGIN
Declare Var_CreditLimit Decimal (10,2);
Select Par_CustomerNumber;
Select CreditLimit into Var_CreditLimit from Customers where CustomerNumber = Par_CustomerNumber;
If Var_CreditLimit >100000 then Select 'Platinum' as CustomerLevel;
ElseIf Var_CreditLimit between 25000 and 100000 Then Select 'Gold' as CustomerLevel;
    Else Select 'Silver' as CustomerLevel;
End if;
END
...
Delimiter $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `Get_Country_Payments`(Par_Year INT, Par_Country CHAR(30))
BEGIN
    SELECT 
        YEAR(PaymentDate) AS Year,Country,CONCAT(FORMAT(SUM(Amount/1000), 0), 'K') AS TotalAmount
    FROM 
        Payments
    JOIN 
        Customers 
    ...
    WHERE 
        YEAR(PaymentDate) = Par_Year AND Country = Par_Country
    GROUP BY 
        YEAR(PaymentDate), Country;
END
Delimiter ;
...
Select Year(OrderDate) as Year, MonthName(OrderDate) as Month, Count(OrderNumber) as TotalOrders, 
Concat(Round(((COUNT(OrderNumber) / LAG(COUNT(OrderNumber)) OVER (ORDER BY YEAR(OrderDate))) - 1)*100,0),'%') AS YoYPercentageChange
from Orders 
...
Delimiter $$
CREATE DEFINER=`root`@`localhost` FUNCTION `Calculate_Age`(Par_DOB Date) RETURNS char(255) CHARSET utf8mb4
BEGIN
Declare Var_Age char(255);
Select Concat(Left(Round(Timestampdiff(Month,Par_DOB,NOW())/12,1),2)," Years ",Right(Round(Timestampdiff(Month,Par_DOB,NOW())/12,1),1)," Months") INTO Var_Age ;
RETURN Var_Age ;
END

Delimiter ;

Select Emp_ID,Name,DOB,Concat(Left(Round(Timestampdiff(Month,DOB,NOW())/12,1),2)," Years ",Right(Round(Timestampdiff(Month,DOB,NOW())/12,1),1)," Months") as Age from Emp_UDF ;
...
Select C.CustomerNumber, C.CustomerName, Count(OrderNumber) as TotalOrders from Customers AS C Left join Orders AS O on C.CustomerNumber = O.CustomerNumber GROUP BY CustomerNumber
Union
Select C.CustomerNumber, C.CustomerName, Count(OrderNumber) as TotalOrders from Customers AS C Right join Orders AS O on C.CustomerNumber = O.CustomerNumber GROUP BY CustomerNumber;


/*Day13* "Question3"*/

Select OrderNumber, Max(QuantityOrdered) as Second_Highest_Quantity_Ordered from OrderDetails where(OrderNumber, QuantityOrdered) 
NOT IN (Select OrderNumber,Max(QuantityOrdered) as Highest_Quantity_Ordered from OrderDetails Group by OrderNumber) 
Group by OrderNumber ; 


/*Day13* "Question4"*/

Select Max(No_of_Products) as MaximumTotal, Min(No_of_Products) as MinimumTotal from (Select OrderNumber, Count(ProductCode) as No_of_Products from OrderDetails group by OrderNumber) as OrderCount;
...
GROUP BY
    productLine Order by Count(Productline) Desc;  
...
Delimiter $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `Emp_EH_Values`(Par_EmpID INT,Par_EmpName VARCHAR(255),Par_EmailAddress VARCHAR(255))
BEGIN
    DECLARE EXIT HANDLER FOR SQLState '42S02'
    BEGIN
     SELECT 'Error occurred' AS Message;  
    END;
    Select * FROM Emp_EH where EmpID = Par_EmpID;
    Select * from Emp_EH WHERE EmpName = Par_EmpName;
    Select * from Emp_EH WHERE EmailAddress = Par_EmailAddress ;
END
